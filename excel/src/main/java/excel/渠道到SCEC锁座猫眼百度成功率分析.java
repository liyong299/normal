...
public class 渠道到SCEC锁座猫眼百度成功率分析
{
	// 	所有渠道锁座	所有渠道成功	
	static String[] title = {  "时间点", "地面超时", 
		...
		"百度锁座", "百度成功", "百度240", 
		"全部成功率","猫眼成功率" ,"百度成功率"};
	static int effectiveLen = 9;
	static int startEffectIdx = 2;

	static String DIR_PATH = "E:/01_work/02_cec/03_需求/04_NGC/07_NGC锁座分析/渠道到NGC猫眼百度/";
	...
	public static void main(String[] args)
	{
		...
		try
		{
			fileName = DIR_PATH + "渠道到NGC锁座猫眼百度成功率分析_" + str + ".xlsx";
	...
	private static Map<String, String[][]> parse(Map<String, String[]> content)
	{
		...
		try
		{
			...
			for (Map.Entry<String, String[]> entry : content.entrySet())
			{
				...
				// 保存每行记录和每次计算结果
				String[] result = new String[value.length + 7];
				...
				// date, min, allNum, all001Num, all240Num, myAllNum, my001Num, my240Num , bdAllNum, bd001Num, bd240Num
				// 所有渠道成功率
				...
				result[effectiveLen + 4] = df.format((Double.valueOf(value[6]) / Integer.valueOf(value[5]) * 100))
						+ "%";
				// 百度成功率
				result[effectiveLen + 5] = df.format((Double.valueOf(value[9]) / Integer.valueOf(value[8]) * 100))
						+ "%";
	...
	private static Map<String, String[]> readSrc(String srcFile)
	{
		...
		try
		{
			...
			for (String line : list)
			{
				...
				String bdAllNum = st.nextToken();
				String bd001Num = st.nextToken();
				String bd240Num = st.nextToken();
				...
				{
					content.put(date + "" + min, new String[] { date, min, allNum, all001Num, all240Num
							, myAllNum, my001Num, my240Num 
							, bdAllNum, bd001Num, bd240Num });
				}
			}
		}
		catch (Exception e)
