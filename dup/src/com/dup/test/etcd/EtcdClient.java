...
{
	static final HttpAsyncClient httpClient = buildDefaultHttpClient();
	...
	/**
	 * 采用http异步调用执行，将结果转换成EtcdResult
	 * @param request
	 * @param expectedHttpStatusCodes
			...
			final int... expectedErrorCodes) throws EtcdClientException
	{
		...
		ListenableFuture<EtcdResult> future = Futures.transform(json, new AsyncFunction<JsonResponse, EtcdResult>()
		{
	...
	private EtcdResult parseEtcdResult(String json) throws EtcdClientException
	{
		...
		try
		{
			result = JSONObject.parseObject(json, EtcdResult.class);
		}
		catch (Exception e)
		{
...
//			List<EtcdResult> ret = new ArrayList<EtcdResult>();
			List<EtcdResult> ret = JSONArray.parseArray(response.json, EtcdResult.class);
//			for (int i = 0; i < array.size(); i++)
//			{
////				EtcdResult next = gson.fromJson(array.get(i), EtcdResult.class);
//				EtcdResult next = JSONArray.P;
//				ret.add(array.get(i));
//			}
			return ret;
		}
	...
	/**
	 * 将异步Client的调用结果，进行转换转成JsonResponse
	 * @param request
	 * @param expectedHttpStatusCodes
	 * @return
	 * @throws EtcdClientException
	...
	/**
	 * 通过apache的HttpAsyncClient客户端，进行etcd调用，执行调用命令，并将结果放入SettableFuture返回。
	 * @param request
	 * @return
	 */
	protected ListenableFuture<HttpResponse> asyncExecuteHttp(HttpUriRequest request)
	{
		...
		Future<HttpResponse> future2 = httpClient.execute(request, new FutureCallback<HttpResponse>()
		{
