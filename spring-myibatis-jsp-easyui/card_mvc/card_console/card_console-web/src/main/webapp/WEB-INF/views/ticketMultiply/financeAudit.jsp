...
<easyui:templateOverride name="body">
	<form id="info" class="edit_box"  method="post" action="">
        <div class="items">
			...
			<input name="name" value="${saleCommonTicket.name }" class="easyui-textbox easyui-validatebox" missingMessage="通兑票名称不能为空" required="true" type="text" />
        </div>
         <div  class="items">
         ...
         <div  class="items">
			...
			<input name="salePrice" value="${saleCommonTicket.sale_price}" class="easyui-textbox easyui-validatebox" required="true" type="text" validType="length[0,32]" missingMessage="销售价不能为空"/>：元
        </div>
        <%--  <div  class="items">
        	<label style="width: 120px" for="itemName">结算价<em>*</em>：</label> 
			<input name="settlPrice" value="${saleCommonTicket.settl_price}" class="easyui-textbox easyui-validatebox" required="true" type="text" validType="length[0,32]" missingMessage="结算价不能为空"/>
			
        </div> --%>
         <div  class="items">
        	<label style="width: 120px;" for="itemName">有效期<em>*</em>：</label> 
			<div style="display:inline-block;">
				固定天数<input id="valid_type1" name="validType" value="0"  type="radio" class="vradio" <c:if test="${saleCommonTicket.valid_type == 0 || saleCommonTicket.valid_type ==null}">checked</c:if> /> <input name="validDays" id="validDays" value="${saleCommonTicket.valid_days }" class="easyui-textbox" type="text" />
				<br />
				时间范围<input id="valid_type2" name="validType" value="1"  type="radio" class="vradio" <c:if test="${saleCommonTicket.valid_type == 1 }">checked</c:if>> <input id="dd" type="text" name="validStime" value="${ saleCommonTicket.valid_stime}" class="easyui-textbox" ></input> 到:<input id="dd" name="validEtime" value="${ saleCommonTicket.valid_etime}" type="text" class="easyui-textbox" ></input> 
        ...
        <%--  <div  class="items">
			...
			<input name="isPpecialPrice" value="1" type="checkbox" <c:if test="${saleCommonTicket.is_special_price == 1 }">checked</c:if> />
        </div> --%>
         <div  class="items">
         ...
         <div  class="items">
        	...
        	<textarea disabled  rows="3" cols="40" class="textbox easyui-validatebox" name="posPosition">${saleCommonTicket.pos_position }</textarea>
        </div>
         <div  class="items">
        	<label style="width: 120px" for="itemName">凭证下发辅助说明：</label> 
			<textarea disabled  rows="3" cols="40" class="textbox easyui-validatebox" name="validateMemo">${saleCommonTicket.validate_memo }</textarea>
        </div>
        <div  class="items">
        	<label style="width: 120px" for="itemName">凭证打印说明：</label> 
			<textarea disabled  rows="3" cols="40" class="textbox easyui-validatebox" name="printMemo">${saleCommonTicket.print_memo }</textarea>
        </div>
        <div  class="items">
        	<label style="width: 120px" for="itemName">网站显示备注：</label> 
			<textarea disabled  rows="3" cols="40" class="textbox easyui-validatebox" name="webMemo">${saleCommonTicket.web_memo }</textarea>
        </div>
        <div  class="items">
        ...
        <input type="hidden" name="code" value="${saleCommonTicket.code}">
         ...
         <div class="item item_row button" style="text-align: center">
            ...
            <a class="easyui-linkbutton" onclick="Save(-2)">财务审核拒绝通过</a>
