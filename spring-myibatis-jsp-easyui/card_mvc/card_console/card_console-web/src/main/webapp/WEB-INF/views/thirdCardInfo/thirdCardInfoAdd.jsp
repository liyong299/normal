...
<easyui:templateOverride name="title">添加第三方卡</easyui:templateOverride>
<easyui:templateOverride name="head">
</easyui:templateOverride>
<easyui:templateOverride name="body">
	<form id="info" class="edit_box"  method="post" action="">
		<input id="id" name="id" value="${thirdCardInfo.id }"  type="hidden" />
        <div class="item"><label>卡BIN：</label><easyui_ext:textBox id="cardBin" name="cardBin"  /><span class="star">*</span></div>
        <div class="item"><label>卡名称：</label><easyui_ext:textBox id="cardName" name="cardName"  /><span class="star">*</span></div>
        <div class="item"><label>结算机构：</label><easyui_ext:comboBox id="settlOrgzainationId" name="settlOrgzainationId"   data="${results}" /><span class="star">*</span></div>
        <div class="item"><label>分成方式：</label><easyui_ext:comboBox id="feeType" name="feeType" data="<%= com.mopon.helpers.ui.combobox.Data.getFeeType()%>" /><span class="star">*</span></div>
        <div class="item"><label>分成比例(%)：</label><easyui_ext:textBox id="feeRate" name="feeRate"  /><span class="star">*</span></div>
	...
	function doSave() {	
		if(validata()){
			$.ajax({
	            cache: true,
	            type: "POST",
	            url:"../thirdCardInfo/saveThirdCardInfo.do",
	            data:$('#info').serialize(),
	            dataType: "json",
	            async: false,
