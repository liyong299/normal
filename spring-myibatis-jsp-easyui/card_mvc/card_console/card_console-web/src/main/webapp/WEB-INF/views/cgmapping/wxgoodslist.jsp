...
<easyui:templateOverride name="body">
	<easyui:layout id="main" fit="true">
		<easyui_ext:searchCondition id="search-form">				
				...
				<easyui_ext:searchItem name="微信产品类型"><easyui_ext:textBox id="wxgoodstype" value="" ></easyui_ext:textBox></easyui_ext:searchItem>						
		</easyui_ext:searchCondition>
		<easyui:layoutUnit region="center" border="false">
			<easyui:datagrid id="datalist" url="wxgsmplist.do" fit="true" pageSize="30" pagination="true" singleSelect="true" >
				<easyui:facet name="toolbar">
			        ...
			        <easyui:button id="btn_edit" plain="true" text="编辑" iconCls="icon-edit"  onclick="editWXgsmap()"></easyui:button>
			        <easyui:button id="btn_enable" text="删除" plain="true" iconCls="icon-delete" onclick="removeWXgsmap()"></easyui:button>
			    </easyui:facet>
			    <easyui:columns>
			        ...
			        <easyui:column field="wxgoodstype" title="微信产品类型" width="100px"/>
			    </easyui:columns>
			</easyui:datagrid>
		</easyui:layoutUnit>
	</easyui:layout>
...
<easyui:templateOverride name="script">
	...
	var datalist = {
	        ...
	        Add: function(){
	        	$("#layout_box iframe").attr("src", "../cgmapping/wxgoodsadd.html");
	            $('#layout_box').window({
	                title: "添加微信产品映射"
	            });
	        },
	...
	function removeWXgsmap()
	{
		var obj = $('#datalist').datagrid('getSelections')[0];
		if(obj == null){
			$.messager.alert('信息提示','请选择要删除的微信产品映射信息！','success',null);
			return;
		}
		var id = obj.id;
		
		$.messager.confirm("提示","确认删除该微信产品映射吗？",function(data){
			if(data){				
				$.ajax({
					type: "POST",
					data:"id="+id,
					url:"../cgmapping/deletewxgoods.do" ,			            
		            dataType: "json",
		            async: false,
	...
	function editWXgsmap()
	{
		var obj = $('#datalist').datagrid('getSelections')[0];
		if(obj == null){
			$.messager.alert('信息提示','请选择要编辑的微信产品映射信息！','success',null);
			return;
		}
		var id = obj.id;
		
		$("#layout_box iframe").attr("src", "../cgmapping/wxgoodsedit.html?id=" + id);
        $('#layout_box').window({
            title: "编辑微信产品映射"
        });			
	}
