...
<easyui:templateOverride name="body">
	<easyui:layout id="main" fit="true">
		<easyui_ext:searchCondition id="search-form">
				<easyui_ext:searchItem name="QQ影院编号"><easyui_ext:textBox id="qqcinemano" value=""></easyui_ext:textBox></easyui_ext:searchItem>
		...
		<easyui:layoutUnit region="center" border="false">
			<easyui:datagrid id="datalist" url="qqcmplist.do" fit="true" pageSize="20" pagination="true" singleSelect="true" >
				<easyui:facet name="toolbar">
			        ...
			        <easyui:button id="btn_edit" plain="true" text="编辑" iconCls="icon-edit"  onclick="editQQcmmap()"></easyui:button>
			        <easyui:button id="btn_enable" text="删除" plain="true" iconCls="icon-delete" onclick="removeQQcmmap()"></easyui:button>
			    </easyui:facet>
			    <easyui:columns>
			        <easyui:column field="ck" checkbox="true"/>
			        <easyui:column field="id" title="ID"/>
			        <easyui:column field="qqcinemano" title="QQ影院编码"  width="100px"/>
			        <easyui:column field="cinemano" title="兜有影院编码"  width="100px"/>
			        <easyui:column field="cinemaname" title="影院名称"  width="200px"/>
			        <easyui:column field="cinemalinename" title="所属院线" width="150px"/>
	...
	<div id="layout_box" style="overflow:hidden" data-options="width: 650,height: 500,closed: false,cache: false,modal: true,minimizable:false,maximizable:true,collapsible:false"><iframe frameborder="0" width="100%" height="100%"></iframe></div>
...
<easyui:templateOverride name="script">
	...
	var datalist = {
	        ...
	        Add: function(){
	        	$("#layout_box iframe").attr("src", "../cgmapping/qqcinemaadd.html");
	            $('#layout_box').window({
	                title: "添加QQ影院映射"
	            });
	        },
		...
		function editQQcmmap()
		{
			var obj = $('#datalist').datagrid('getSelections')[0];
			if(obj == null){
				$.messager.alert('信息提示','请选择要编辑的QQ影院映射信息！','success',null);
				return;
			}
			var id = obj.id;
			
			$("#layout_box iframe").attr("src", "../cgmapping/qqcinemaedit.html?id=" + id);
            $('#layout_box').window({
                title: "编辑QQ影院映射"
            });			
		}
		
		function removeQQcmmap()
		{
			var obj = $('#datalist').datagrid('getSelections')[0];
			if(obj == null){
				$.messager.alert('信息提示','请选择要删除的QQ影院映射信息！','success',null);
				return;
			}
			var id = obj.id;
			$.messager.confirm("提示","确认删除该QQ影院映射吗？",function(data){
				if(data){
					
					$.ajax({
						type: "POST",
						data:"id="+id,
						url:"../cgmapping/deleteqqcinema.do" ,			            
			            dataType: "json",
			            async: false,
