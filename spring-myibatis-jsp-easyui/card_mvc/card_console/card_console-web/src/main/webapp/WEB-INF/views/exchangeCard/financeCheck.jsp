...
<easyui:templateOverride name="body">
	<div class="easyui-layout" fit="true">
		...
		<div data-options="region:'south'" style="border:0">
			<div class="edit-box">
				<div class="bottom">
					<c:if test="${template.status == 3}">
					<a class="easyui-linkbutton" onclick="operation.FinanceCheckOK()">财务审核通过</a>
					<a class="easyui-linkbutton" onclick="operation.FinanceCheckNO()">财务审核拒绝</a>
					</c:if>
...
<easyui:templateOverride name="script">
	...
	<script>
		var operation = {
		        ...
		        FinanceCheckOK: function(){
		        	$.messager.confirm("系统提示", "确定审核通过？", function (b) {
		                 if (b) {
		                	 $.ajaxRequest({
		 		        		url: "financeCheckOK.do",
		        ...
		        FinanceCheckNO: function(){
		        	$.messager.confirm("系统提示", "确定审核拒绝？", function (b) {
		                 if (b) {
		                	 $.ajaxRequest({
		 		        		url: "financeCheckNO.do",
		 		        		para: {templateid: $("#templateid").val()},
		 		        		success: function(result){
		 		        			if(result.isSuccess){
		 		        				parent.operation._temp.list_box.datagrid("reload");
