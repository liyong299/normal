 ...
 * @author ly
 */
public class CompleteBinaryTree<K, V> {
	...
	public CompleteBinaryTree(TreeNode<K, V> root) {
		this(root, DEFAULT_SIZE);
	}

	public CompleteBinaryTree(TreeNode<K, V> root, int size) {
	...
	public TreeNode<K, V>[] preorderTraversal() {
	...
	private void preorderTraversal(TreeNode node, List<TreeNode<K, V>> list) {
		if (node != null) {
			list.add(node);
	...
	public TreeNode<K, V>[] suforderTraversal() {
		List<TreeNode<K, V>> list = new LinkedList<TreeNode<K, V>>();
		suforderTraversal(root, list);
		TreeNode[] tmp = new TreeNode[list.size()];
		return list.toArray(tmp);
	}

	private void suforderTraversal(TreeNode node, List<TreeNode<K, V>> list) {
		if (node != null) {
			inorderTraversal(node.getLeftNode(), list);
			inorderTraversal(node.getRightNode(), list);
			list.add(node);
